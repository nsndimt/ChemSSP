...
class UniNERExample:
    ...
    def parseResult(self, response):
        ...
        try:
            ...
            response = response[start:end].replace('\n', ' ')
        except Exception:
            # print(response)
            return None
    ...
    def toRequest(self):
        conversation = self.template.format(input_passage=self.text)
        request_json = {
...
def run_claude_api(requests_filepath, save_filepath):
    ...
    @backoff.on_exception(
            backoff.expo,
            (ResourceExhausted, TooManyRequests, InternalServerError, ServiceUnavailable),
            max_tries=60, max_value=1800)
...
if __name__ == '__main__':
    ...
    if args.create_task:
        ...
        request_fn = f'gemini/request.jsonl'
        with open(request_fn, 'w') as f:
            for sample in samples:
                f.write(sample.toRequest() + '\n')

    if args.call_api:
        request_fn = 'gemini/request.jsonl'
        result_fn = 'gemini/result.jsonl'
        run_claude_api(request_fn, result_fn)

    if args.parse_result:
        result_fn = 'gemini/result.jsonl'
        sent_fn = 'gemini/sentence.jsonl'
    ...
    if args.sample_episode:
        sent_fn = f'gemini/sentence.jsonl'
        sample_episode(sent_fn, args.mode, 5000, args.UniNway, args.UniKshot,
                       args.UniQtest, args.UniTopK, args.UniAlpha,
                       just_sample=False, output_dir="gemini")

    if args.filter_episode:
        filter_large_episode(args, 25000, output_dir="gemini")
